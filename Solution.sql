...
-- 1. Count the number of Movies vs TV Shows
     ...
     SELECT type,rating,COUNT(*),RANK() OVER(PARTITION BY type ORDER BY COUNT(*) DESC) AS Total_Con FROM netflix GROUP BY type;
...
-- 4. Find the top 5 countries with the most content on Netflix.
    ...
    SELECT * FROM
    (SELECT UNNEST(STRING_TO_ARRAY(country, ',')) as country,COUNT(*) as total_content FROM netflix GROUP BY 1)as t1
...
-- 7. Find all the movies/TV shows by director 'Rajiv Chilaka'.
     ...
     SELECT *
     FROM
     (SELECT *,UNNEST(STRING_TO_ARRAY(director, ',')) as director_name FROM netflix) WHERE director_name = 'Rajiv Chilaka';
...
-- 10.Find each year and the average numbers of content release in India on netflix. return top 5 year with highest avg content release!.
   ...
   SELECT 
	...
	as avg_release
    FROM netflix WHERE
	country = 'India' GROUP BY country, 2
